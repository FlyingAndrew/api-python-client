...
class SOS:
    ...
    def getCapabilities_SOAP(self):
        ...
        if (response.ok):
            ...
            operation = dict['{0}Capabilities'.format(sos)]['{0}OperationsMetadata'.format(ows)][
                '{0}Operation'.format(ows)]
    ...
    def getObservation_SOAP(self, offering, begin, end, observedProperty, propertyDictionary={}):
        ...
        # headers = {'content-type': 'text/xml'}
        observedProperties = '\n'.join(
            ['                    <observedProperty>urn:ogc:def:phenomenon:{}</observedProperty>'.format(p) for p in
             observedProperty])
